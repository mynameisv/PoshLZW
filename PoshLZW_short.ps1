...
function lzwc{param($d,$z)$t=0;$k=($y=@());$r=$w='';$a=[array];foreach($i in 0..255){$y+=[string][char]$i}$x=$m_::Pow(2,$z)-1;for($i=0;$i-lt$d.length;$i++){if($a::IndexOf($y,($w=$r+[char]$d[$i]))-gt-1){$r=$w}else{bw ($a::IndexOf($y,$r)) $z ([ref]$t) ([ref]$k);if($y.count-ge$x){$r=[string][char]$d[$i];}else{$y+=$w}$r=[string][char]$d[$i];}}bw ($a::IndexOf($y,$r)) $z ([ref]$t) ([ref]$k);return @($z)+$k;}