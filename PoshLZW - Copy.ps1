...
if ($Datas){
	# Array of bytes ?
	if ($Datas.GetType().fullname -eq "System.Byte[]"){
		# Ok...
		$aDatas = $Datas;
	} else {
		# If compression, it has to be a simple string
		# If decompression, it has to be a base64 encoded string
		if ($Compress){
			if ($Datas..GetType().fullname -eq "System.String"){
				# Ok
				$aDatas = [System.Text.Encoding]::UTF8.GetBytes($Datas);
			} else {
				write-host " [!] Error, input datas type not bytes nor string.";
				exit;
			}
		} elif ($Decompress){
			# Two ways to check : check each character part of the b64 alphabet or... try/catch to debase64 ;-)
			Try {
				$aDatas = [System.Text.Encoding]::Unicode.GetString([Convert]::FromBase64String($Datas));
			} Catch {
				write-host " [!] Error, datas seems to be Base64 encoded but can not be decoded.";
				exit;
			}
		} else {
			write-host " [!] Error, no action to do.";
		}
	}
	...
	decompress data form B64, byte array
